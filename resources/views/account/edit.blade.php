...
<div class="card">
  <div class="card-body add-product pb-0">
    <form action="{{ route('accounts.update',$account->id) }}" method="POST" class="row g-3">
      @csrf
      @method('PUT')
      <div class="col-lg-4 col-sm-6 col-12">
        <div class="mb-3 add-product">
          <label class="form-label">Name</label>
          <input type="text" name="name" class="form-control" value="{{ $account->name }}" required placeholder="Enter Account Name">
        </div>
      </div>
      <div class="col-lg-4 col-sm-6 col-12">
        <div class="mb-3 add-product">
          ...
          <select name="type" class="select form-select" required>
            ...
            <option value="mobile" @selected($account->type==='mobile')>Mobile</option>
            <option value="other" @selected($account->type==='other')>Other</option>
          </select>
        </div>
      </div>
      <div class="col-lg-4 col-sm-6 col-12">
      ...
      <div class="col-lg-4 col-sm-6 col-12">
        <div class="form-check mt-4">
          <input class="form-check-input" type="checkbox" name="is_default" value="1" id="is_default" @checked($account->is_default)>
          <label class="form-check-label" for="is_default">Default account</label>
        </div>
      </div>
      <div class="col-lg-4 col-sm-6 col-12">
        <div class="form-check mt-4">
          <input class="form-check-input" type="checkbox" name="is_active" value="1" id="is_active" @checked($account->is_active)>
